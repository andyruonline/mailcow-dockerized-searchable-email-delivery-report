...
def process_logs(lines, search_term, search_type, date_filter, time_filter=None):
    ...
    for line in lines:
        ...
        # Apply search filter based on type (skip if search term is blank)
        if search_term:
            if search_type == "sender":
                from_field = (extract_field(line, r'from=<([^>]*)>') or "").lower()
                if search_lower not in from_field:
                    continue
            elif search_type == "recipient":
                to_field = (extract_field(line, r'to=<([^>]*)>') or "").lower()
                if search_lower not in to_field:
                    continue
            else:  # both
                from_field = (extract_field(line, r'from=<([^>]*)>') or "").lower()
                to_field = (extract_field(line, r'to=<([^>]*)>') or "").lower()
                if not (search_lower in from_field or search_lower in to_field):
                    continue
...
Examples:
  ...
  %(prog)s --test --days 2
    ...
    if interactive_mode:
        ...
        date_filter = input("Filter by date (e.g. '5 Dec', 'Dec 5', '5 Dec 2025', or '3' for last 3 days, or blank): ").strip()
    ...
    # Process logs
    messages = process_logs(lines, search_term, search_type, date_filter, time_filter)
